        ...
        , SensorEventListener
{
    ...
    // CSV 파일명
    private static String GPS_CSV = "_gps.csv";
    private static String NOISE_CSV = "_noise_.csv";
    private static String RPM_CSV = "_rpm.csv";
    // CSV 파일 경로
    ...
    public void sendJsonData(String url) {
        ...
        try {
            ...
            // csv 파일 생성
            List<String[]> csvGps = csvFile.readAllCsvData(carNum + "_" + date + GPS_CSV);
            csvGps.add(new String[]{carNum, "" + date, "" + time, "" + mLat, "" + mLng});
            csvFile.writeData(carNum + "_" + date + GPS_CSV, csvGps);
            List<String[]> csvNoise = csvFile.readAllCsvData(carNum + "_" + date + NOISE_CSV);
            csvNoise.add(new String[]{carNum, "" + date, "" + time, "" + amplitude});
            csvFile.writeData(carNum + "_" + date + NOISE_CSV, csvNoise);
            List<String[]> csvRpm = csvFile.readAllCsvData(carNum + "_" + date + RPM_CSV);
            csvRpm.add(new String[]{carNum, "" + date, "" + time, "" + rpm});
            csvFile.writeData(carNum + "_" + date + RPM_CSV, csvRpm);
